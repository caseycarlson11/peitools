...
- Shipment (packing list) colors must be DISTINCT per list and must NEVER include red — red is reserved for the Panel Mapper's panel boxes. Palettes: `_SHIPMENT_COLORS` (packing_list_engine.py) and `SHIP_COLORS[]` (packing_list_editor.html) — keep them the same order/length (no red). New packing lists get the lowest unused color via `shipIdx()` in the editor.
