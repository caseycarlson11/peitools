...
- Output table titled "DELIVERED", top-right corner, auto-sized columns, legend strip at bottom. Layout uses AS MANY side-by-side panel columns as needed (1, 2, 3, …) so every panel fits within the page height (`n_data_cols = ceil(n / avail_rows)` in `_insert_delivery_table`) — the table grows leftward instead of running off the bottom
