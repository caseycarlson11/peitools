...
9. **Public Links** (`/public-links`) — One permanent PUBLIC (no-login) link per document slot, per job. Seven slots (the seventh, To-Do Board, serves no file — its `/pl/<token>` 302-redirects to the To-Do Channel Viewer `/todo/<view_token>`; publish just ensures the view token exists; `/pl/<token>/file` 404s for it; job_page lists it when live): Marked-Up Prints + Panels-Only Prints (hand-picked PDF from Blueprints/Panel Mapper folders), All Packing Lists + All Fab Sheets (auto-merged single PDF of the whole folder, rebuilt automatically when contents change, bookmark per source file, cached in `<job>/Public Links/`), Job Spreadsheet (SheetJS viewer + Download + Open-in-Google-Sheets using the job's linked `_sheets_url.json`, copies TSV for pasting), and Job Page (public landing page listing all live docs). Publish = swap the document behind the link (the LINK NEVER CHANGES); New Link = rotate token, old link dies instantly (access kill switch); Disable = kill without replacement. Every live slot also has a "🔳 QR Code" button → overlay with a scannable code + Download PNG (`/api/qr?data=&name=&dl=1`, login-gated, `qrcode` lib server-side); a printed QR encodes the URL, so rotating the link kills printed codes too — reprint after New Link. Store: `<JOBS_DIR>/.public_links.json` = `{job: {slot: {token, file, published}}}`. Public routes: `/pl/<token>` (viewer) and `/pl/<token>/file` (raw, `?dl=1` = download). Templates: `public_links.html` (admin), `public_doc_view.html`, `public_sheet_view.html`, `public_job_page.html`. Panel Tracking's Overview page shows a read-only "Public Links" card (all six slots, live links with Copy/Open, under the Fab Sheets card) via `/api/public-links/<job>`; managing/publishing still happens in this tool. NOT yet wired into Panel Tracking's publish buttons (planned next).
...
11. **Push to Discord** (`/field-report` — route kept from its original "Field Report" name) — Mobile-first tool for foremen. Photos are compressed client-side via canvas; formats the browser can't decode (iPhone HEIC on desktop) are NEVER silently dropped — they get a 🖼 placeholder tile and upload as originals, and the server's `_fr_to_jpeg` (pillow-heif + Pillow) converts any non-JPEG upload to a resized JPEG before posting to Discord. Tool details: take/pick photos (compressed client-side to ≤1600px JPEG), write or dictate a note (two dictation engines: if `<JOBS_DIR>/.openai_key.txt` holds an OpenAI API key, the Dictate button records audio via MediaRecorder and transcribes server-side at `/api/field-report/transcribe` (model `whisper-1` — NOT gpt-4o-*-transcribe, those hallucinate on weak audio; trade-term prompt bias, `language=en`, 5-min/20MB cap). Phone recordings (esp. iPhone Safari) arrive ~25dB too quiet, so `_fr_normalize_audio` (PyAV+numpy) decodes and boosts every clip to -3dBFS peak / 16kHz mono WAV before sending; falls back to raw audio if decoding fails. Last clip + result kept at `<JOBS_DIR>/.last_voice_note.{bin,json}`, downloadable via `/api/field-report/last-audio` (`?info=1` for metadata) for debugging bad transcriptions — best accuracy, same on every device; with no key, `/api/field-report/config` reports `transcribe:false` and the page falls back to the free Web Speech engine with auto-restart-on-silence; iOS keyboard mic always works), pick a Discord channel, and post the photos + note as a formatted embed straight to that channel's Discord **webhook**. Push-only (nothing stored on the site). **One Discord SERVER per job.** The tool flow is: pick job → pick channel → send — but the job card AUTO-HIDES when only ONE server is configured (steps renumber via the `.step-*` spans; the single job is used silently). Channel keys/labels are `DISCORD_CHANNELS` in app.py (7 channels: gc, pm, todo, safety, progress, extra_work, trade_damage — keep in sync with the actual Discord server; the BOT token can list real channel names via `GET /guilds/<id>/channels` when they drift); webhook config is `<JOBS_DIR>/.discord_webhooks.json` shaped `{job: {channel_key: url}}` — the original server belongs to "Modesto Courthouse"; each job server only needs entries for the channels it actually has (legacy flat `{channel: url}` files auto-load as Modesto Courthouse). Public Links shows the To-Do Board slot ONLY for jobs whose server has a `todo` webhook (state API omits it otherwise; old completion records with a stale job key heal to the single configured server). The SAME bot token works across all job servers but the bot must be INVITED to each one. The file lives ON THE PERSISTENT JOBS VOLUME (like `.public_links.json`) because neither deploy script ships untracked root files: deploy_quick.bat tars a fixed file list and deploy.bat goes through git, where the file must never appear (secrets). Locally that's `Jobs\.discord_webhooks.json`; on the server `/var/www/pei-jobs/.discord_webhooks.json`; `upload_webhooks.bat` scp's local→server after edits. `discord_webhooks.example.json` is the committed template. A channel chip shows "not set up" until that job has its URL. Reports to a job server get the job name in the embed title; completion records store `job` for the webhook lookup. Routes: `/field-report` (page), `/api/field-report/channels` (which channels are configured), `/api/field-report/send` (multipart: `channel`, `note`, `photos[]`), `/fr/<token>` (task-complete click). **Urgent (To-Do only):** an "🚨 Mark as URGENT" toggle appears under the chips only when To-Do is selected (`urgent=1` form field, ignored for other channels server-side); urgent embeds get title "🚨 URGENT Field Report…" + red color 0xF85149, and the web board shows a red bar/title (`.embed.urgent`, detected by "URGENT" in the title when not done). Completing the task strips the urgent title back to 📋 and turns it green. **Task checkbox:** ONLY To-Do messages end with a "☐ Mark task complete" link (other channels post plain reports, no token/record); the link is (real Discord buttons need a bot app, not webhooks). Tapping it opens `/fr/<token>` — PUBLIC, no login (unguessable-token trust model like Public Links). The GET page changes NOTHING; its inline JS fires `POST /fr/<token>/complete` in the background — this two-step exists so link-preview crawlers (Discord unfurler, iOS, mail scanners) fetching the URL can't auto-complete tasks; keep it. The POST PATCHes the webhook message in place — checkbox line becomes "✅ Completed by <name> · <t:unix:f>" and embed turns green — then records completer+time. First click wins; later clicks show "already completed by X". Name = PEI login if a session exists (`_fr_display_name`: caseyc→"Casey C."), else "a crew member". Records (token → channel/message_id/embed/completed_by) live in `<JOBS_DIR>/.field_reports.json` (jobs volume, capped at newest 400). Send uses `?wait=true` to capture the message id; absolute link uses `request.url_root` (same pattern as Public Links). DEV NOTE: on Casey's Windows machine, Python processes spawned by Claude cannot write inside the project/Jobs folders (writes fail as FileNotFoundError) — test persistence on the server, or mock `_fr_load_reports`/`_fr_save_reports`. Adding/removing a channel = edit `DISCORD_CHANNELS` + the JSON. NOTE: Discord *invite* links (discord.gg/...) cannot post to a channel — each channel needs its own Webhook URL (Edit Channel → Integrations → Webhooks → New Webhook → Copy URL). Requires the `requests` library, so a new channel-webhook value just needs `upload_webhooks.bat`, but the first deploy after adding this tool needs the full `deploy.bat` (requirements.txt changed).

12. **To-Do Channel Viewer** (`/todo/<token>` — PUBLIC, no login) — ONE BOARD PER JOB: each token maps to a job (`<JOBS_DIR>/.todo_view_tokens.json` = `{job: token}`) and shows that job server's #to-do channel; header/title show the job name. Web page for people without Discord: messages newest-first with author/time, field-report embeds (blue bar pending / green bar done, tappable "Mark task complete" link, mini-markdown: bold, links, `<t:unix:f>` → local time), inline photos, file links. Auto-refreshes every 60s + Refresh button. `/todo` (login-gated) lists every job that has a to-do webhook with its share link (creates tokens on demand). Reading Discord requires a BOT token (webhooks can't read): `<JOBS_DIR>/.discord_bot_token.txt` — one token for all servers, but the bot must be INVITED to each job server with View Channels + Read Message History (+ Message Content intent in the dev portal). Channel id per job resolved from that job's to-do webhook (GET webhook → channel_id); `_todo_cache` is keyed by job, messages cached 20s. Routes: `/todo`, `/todo/<tok>`, `/api/todo/<tok>/messages`. Template: `todo_view.html` (standalone). Public Links' To-Do Board slot publishes per job and 302s to that job's board.
